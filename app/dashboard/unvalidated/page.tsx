...
export default async function UnvalidatedPage() {
  ...
  const { data } = await supabase
    .from('parts')
    .select('id, part_number, hts_code, validation_status, last_validated_at')
    .eq('org_id', scopeId)
    .in('validation_status', ['not_found', 'error', 'pending'])
    .order('last_validated_at', { ascending: false, nullsFirst: true })
              ...
              <tbody>
                {parts.map((part) => (
                  <tr key={part.id} className="border-t border-slate-100 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className="text-[10.5px] font-medium text-slate-700" style={{ fontFamily: 'var(--font-plex-mono)' }}>
                        {part.hts_code}
                      </span>
                    </td>
                    <td className="px-4 py-3">
